						...
						<div className="relative h-full overflow-hidden rounded-[2.85rem] bg-white">
							...
							<div className="relative flex items-center justify-between px-6 pb-3 pt-7 text-[11px] text-zinc-700">
								...
								<div className="flex items-center gap-2">
									<Dot /><Dot /><Dot />
									<span className="ml-1 rounded-full bg-zinc-900/8 px-2 py-0.5 text-[10px]">inPulse</span>
								</div>
							</div>
									...
									<StackedScreen isActive={show.landing} mounted={mounted}>
										...
										>
											<div className="mt-4 text-sm leading-relaxed text-zinc-700">
												...
												<br />Simple, contextuel, temporaire.
											</div>
										</ScreenCard>
...
function StackedScreen({ isActive, mounted, children }: { isActive: boolean; mounted: boolean; children: React.ReactNode; }) {
	return (
		<div className={[
...
function Dot() { return <span className="inline-block h-1.5 w-1.5 rounded-full bg-zinc-900/35" />; }
...
function ChoiceButton(props: { label: string; onClick: () => void; tint: "primary" | "accent"; }) {
	const glow = props.tint === "accent"
		? "bg-[hsl(var(--ip-accent)/0.20)]"
		: "bg-[hsl(var(--ip-primary)/0.20)]";

	return (
		<button onClick={props.onClick} className="group relative w-full overflow-hidden rounded-2xl border border-zinc-200 bg-white px-4 py-4 text-left active:scale-[0.99]">
